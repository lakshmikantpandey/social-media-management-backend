...
class Linkedin implements social {
	...
	async getUser(params: { Token: string }): Promise<{
		FirstName: string;
		LastName: string;
		Username: string;
		id: any;
		...
		try {
			...
			return {
				...
				Following: 0,
				Username:
					user.data.localizedFirstName + " " + user.data.localizedLastName,
			};
		} catch (error) {
			throw error;
		}
	...
	async createPost(
		...
		try {
			if (IsMediaPost) {
				...
				const uploadUrl =
					uploadRes.value.uploadMechanism[
						"com.linkedin.digitalmedia.uploading.MediaUploadHttpRequest"
					]["uploadUrl"];
				const resBinary = await UrlToBinary(
					params.MediaContent ? params.MediaContent[0].File : "",
				);
				//Image upload
